...
assert.match(storyDirector, /onCompleted\(\)[\s\S]*markLineHeardForSession\(playbackLineKey, playbackSessionId\)/, "recorded Story chapters must be marked heard after playback completion for the same run");
assert.match(storyDirector, /onDone\(String utteranceId\)[\s\S]*markLineHeardForSession\(activeLineKey, activeUtteranceSessionId\)/, "TTS Story chapters must be marked heard after speech completion for the same run");
assert.match(storyDirector, /hasHeardChapter\("contact"\) && completionRatio >= 0\.33d/, "Pursuit narration cannot jump ahead of an unheard Contact chapter");
assert.match(storyDirector, /hasHeardChapter\("pursuit"\) && completionRatio >= 0\.64d/, "Complication narration follows completed Pursuit narration");
assert.match(storyDirector, /hasHeardChapter\("complication"\) && completionRatio >= 0\.84d/, "Extraction narration follows completed Complication narration");
assert.match(storyDirector, /expectedSessionId\.equals\(prefs\.getString\(KEY_SESSION_ID, ""\)\)/, "late narration callbacks from a previous run are discarded");
...
const routeRuntime = source("src/runningRouteRuntime.ts");
assert.match(routeRuntime, /if \(session\.stage === "warmup"\)[\s\S]*buildInitialRoute\(session\)/, "final route generation waits for prep and warm-up GPS");
assert.doesNotMatch(routeRuntime, /if \(session\.stage === "briefing"\) \{\s*void buildInitialRoute/, "briefing does not request the final GPS route");
