...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_zenchad_minddojo_QwenJournalNative_generate(
        JNIEnv *env,
        jclass /*unused*/,
        jstring jmodel_path,
        jstring jprompt,
    ...
    if (!model) {
        cleanup();
        return env->NewStringUTF("__ERROR__: Qwen model could not be loaded.");
    }

    context = init_context(model, DEFAULT_CONTEXT_SIZE);
    if (!context) {
        cleanup();
        return env->NewStringUTF("__ERROR__: Qwen context could not be created.");
    }
    ...
    const bool has_chat_template = common_chat_templates_was_explicit(g_chat_templates.get());
    const std::string formatted_prompt = common_chat_format_single(
            g_chat_templates.get(), chat_msgs, user_message, true, false);
    const auto tokens = common_tokenize(g_context, formatted_prompt, has_chat_template, has_chat_template);
    if (tokens.empty() || (int) tokens.size() >= DEFAULT_CONTEXT_SIZE - OVERFLOW_HEADROOM) {
        cleanup();
        return env->NewStringUTF("__ERROR__: The journal draft is too long for the local context.");
    }
    if (decode_tokens_in_batches(g_context, g_batch, tokens, 0, true) != 0) {
        cleanup();
        return env->NewStringUTF("__ERROR__: Qwen could not process the journal draft.");
    }

    current_position = (llama_pos) tokens.size();
    const int requested_tokens = std::max(1, std::min((int) max_tokens, DEFAULT_CONTEXT_SIZE - current_position - 1));
    std::string output;
    for (int generated = 0; generated < requested_tokens; generated++) {
        ...
        if (llama_decode(g_context, g_batch) != 0) {
            cleanup();
            return env->NewStringUTF("__ERROR__: Qwen stopped while generating the draft.");
        }
        current_position++;
